...
import { Users, Map, Plus, RefreshCw, User, Globe, Trash2, FileText, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function CardsPanel() {
    ...
    const [typeFilter, setTypeFilter] = useState('character'); // 'character', 'world', 'style'

    // Style card state
    const [styleCard, setStyleCard] = useState(null);
    ...
    const loadEntities = async () => {
        setLoading(true);
        try {
            // Parallel fetch characters, world entities, and style
            const [charsResp, worldsResp, styleResp] = await Promise.allSettled([
                ...
                cardsAPI.getStyle(projectId) // 文风卡获取单例
            ]);
    ...
    const handleCreateCard = () => {
        // Create new card based on current filter (not for style)
    ...
    const handleSaveStyle = async () => {
        try {
            await cardsAPI.updateStyle(projectId, styleCard);
        } catch (error) {
    ...
    // Sliding toggle options (only character and world now)
    const typeOptions = [
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2">
                {/* Style Card - Direct Editing */}
                {typeFilter === 'style' && styleCard && (
                        ...
                        {styleExpanded && (
                            <div className="pl-6 pr-2 space-y-3 pb-4">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-bold text-ink-500 uppercase">叙事距离</label>
                                    <input
                                        type="text"
                                        value={styleCard.narrative_distance || ''}
                                        onChange={e => setStyleCard(prev => ({ ...prev, narrative_distance: e.target.value }))}
                                        onBlur={handleSaveStyle}
                                        className="w-full text-xs p-2 border border-border rounded bg-surface/50 focus:border-primary focus:ring-1 focus:ring-primary/20"
                                        placeholder="如：第一人称、全知视角"
                                    />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-bold text-ink-500 uppercase">节奏控制</label>
                                    <input
                                        type="text"
                                        value={styleCard.pacing || ''}
                                        onChange={e => setStyleCard(prev => ({ ...prev, pacing: e.target.value }))}
                                        onBlur={handleSaveStyle}
                                        className="w-full text-xs p-2 border border-border rounded bg-surface/50 focus:border-primary focus:ring-1 focus:ring-primary/20"
                                        placeholder="如：快节奏、缓慢细腻"
                                    />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-bold text-ink-500 uppercase">句式偏好</label>
                                    <textarea
                                        value={styleCard.sentence_structure || ''}
                                        onChange={e => {
                                            setStyleCard(prev => ({ ...prev, sentence_structure: e.target.value }));
                                        ...
                                        className="w-full text-xs p-2 border border-border rounded bg-surface/50 focus:border-primary focus:ring-1 focus:ring-primary/20 min-h-[60px] resize-none overflow-hidden"
                                        placeholder="如：短句为主、长短句结合"
                                    />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-bold text-ink-500 uppercase">词汇约束（每行一个）</label>
                                    <textarea
                                        value={(styleCard.vocabulary_constraints || []).join('\n')}
                                        onChange={e => {
                                            const lines = e.target.value.split('\n').filter(l => l.trim());
                                            setStyleCard(prev => ({ ...prev, vocabulary_constraints: lines }));
                                            e.target.style.height = 'auto';
                                            e.target.style.height = e.target.scrollHeight + 'px';
                                        }}
                                        onBlur={handleSaveStyle}
                                        ...
                                        className="w-full text-xs p-2 border border-border rounded bg-surface/50 focus:border-primary focus:ring-1 focus:ring-primary/20 min-h-[60px] resize-none overflow-hidden font-mono"
                                        placeholder="禁用词汇或偏好词汇..."
                                    />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-bold text-ink-500 uppercase">示例片段（用 --- 分隔）</label>
                                    <textarea
                                        value={(styleCard.example_passages || []).join('\n---\n')}
                                        onChange={e => {
                                            const passages = e.target.value.split('---').map(p => p.trim()).filter(p => p);
                                            setStyleCard(prev => ({ ...prev, example_passages: passages }));
                                            e.target.style.height = 'auto';
                                            e.target.style.height = e.target.scrollHeight + 'px';
                                        }}
                                        onBlur={handleSaveStyle}
                                        ...
                                        className="w-full text-xs p-2 border border-border rounded bg-surface/50 focus:border-primary focus:ring-1 focus:ring-primary/20 min-h-[100px] resize-none overflow-hidden"
                                        placeholder="粘贴符合目标文风的示例文本..."
                                    />
                                </div>
                            </div>
                        )}
